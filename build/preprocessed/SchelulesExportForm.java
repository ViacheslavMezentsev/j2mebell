...
public class SchelulesExportForm implements CommandListener, ItemStateListener {
    ...
    public SchelulesExportForm( MIDlet Мидлет, Displayable Окно, Settings Настройки, Schedules Менеджер ) {
        ...
        Форма = new Form ( "Экспорт расписаний" );
        ...
        main.Логгер.info( "[SchelulesExportForm.java]: SchelulesExportForm()" );
    }
    ...
    public void commandAction( Command команда, Displayable элемент ) {
        ...
        if ( элемент == Форма ) {
            ...
            if ( команда == КомандаЭкспорт ) {
                ...
                main.Логгер.info( "[SchelulesExportForm.java]: <Экспорт>" );

//                final ProgressForm ОкноПрогресса = new ProgressForm( Мидлет, Форма, Настройки, ProgressForm.РЕЖИМ_ЭКСПОРТ );
...
//                    new Runnable() {
//                        public void run() {
                            МенеджерРасписаний.ЭкспортРасписаний( ПолеИмяФайла.getString(0), ПолеКодировка.getSelectedIndex() );

                            String Текст = "Расписания сохранены в файле: \n" + ПолеИмяФайла.getString(0) + "\n" +
                                    "в кодировке ";
...
//                ).start();
                ...
                main.Логгер.info( "[SchelulesExportForm.java]: <Назад>" );
