...
public class EditPlanNameForm implements CommandListener {
    ...
    private PlansForm РодительскоеОкно;
    ...
    public EditPlanNameForm( MIDlet мидлет, PlansForm родитель, Schedules менеджер ) {
        ...
        ПолеТекста = new TextField( "План: ", "", 16, TextField.ANY );
    ...
    public EditPlanNameForm( MIDlet мидлет, PlansForm родитель, Schedules менеджер, int цПлан ) {
        ...
        План = ( Schedules.ОбразПлана ) МенеджерРасписаний.СписокПланов.Элементы.elementAt( цИндекс );

        цРежим = РЕЖИМ_ИЗМЕНИТЬ;
        Форма = new Form ( "Изменить" );

        ПолеТекста = new TextField( "План: ", План.Название, 16, TextField.ANY );
            ...
            if ( команда == КомандаОк ) {
                ...
                switch ( цРежим ) {
                    ...
                    case РЕЖИМ_ИЗМЕНИТЬ:
                        ...
                        Schedules.ОбразПлана План;
                        План = ( Schedules.ОбразПлана ) МенеджерРасписаний.СписокПланов.Элементы.elementAt( цИндекс );
                        План.Название = ПолеТекста.getString();
                        break;

                }

                РодительскоеОкно.Обновить();
                Дисплей.setCurrent( РодительскоеОкно.Список );
